  ...
  describe("BoundPoolClient Trading", () => {
    it("buy meme tokens tx", async () => {
      ...
      const { availableAmountWithDecimals: availableAmountWithDecimalsBefore } =
        await MemeTicketClient.fetchAvailableTicketsByUser2(poolAccountAddressId, client, payer.publicKey);
      ...
      // while (retries > 0) {
      //   const { availableAmountWithDecimals } = await MemeTicketClient.fetchAvailableTicketsByUser2(
    ...
    it.skip("buy and sell meme tokens tx", async () => {
      ...
      const { availableAmountWithDecimals: availableAmountWithDecimalsBeforeBuy } =
        await MemeTicketClient.fetchAvailableTicketsByUser2(boundPoolInstance.id, client, payer.publicKey);
