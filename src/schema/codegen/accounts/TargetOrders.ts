...
export class TargetOrders {
  ...
  static readonly discriminator = Buffer.from([113, 225, 140, 255, 65, 144, 239, 231]);
  ...
  constructor(fields: TargetOrdersFields) {
    ...
    this.buyOrders = fields.buyOrders.map((item) => new types.TargetOrder({ ...item }));
    ...
    this.sellOrders = fields.sellOrders.map((item) => new types.TargetOrder({ ...item }));
  ...
  static async fetch(c: Connection, address: PublicKey): Promise<TargetOrders | null> {
  ...
  static async fetchMultiple(c: Connection, addresses: PublicKey[]): Promise<Array<TargetOrders | null>> {
  ...
  static decode(data: Buffer): TargetOrders {
    ...
    return new TargetOrders({
      owner: dec.owner,
      buyOrders: dec.buyOrders.map((item: unknown) => types.TargetOrder.fromDecoded(item)),
      ...
      sellOrders: dec.sellOrders.map((item: unknown) => types.TargetOrder.fromDecoded(item)),
  ...
  toJSON(): TargetOrdersJSON {
    return {
      ...
      replaceBuyClientId: this.replaceBuyClientId.map((item) => item.toString()),
      replaceSellClientId: this.replaceSellClientId.map((item) => item.toString()),
  ...
  static fromJSON(obj: TargetOrdersJSON): TargetOrders {
    return new TargetOrders({
      ...
      sellOrders: obj.sellOrders.map((item) => types.TargetOrder.fromJSON(item)),
