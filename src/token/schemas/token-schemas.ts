...
export const paginatedResultSchema = <T extends ZodRawShape>(result: z.ZodObject<T>) => z.object({
    paginationToken: z.string().nullish(),
    result: z.array(result)
});

export const solanaTokensSortableColumns = z.literal('marketcap').or(z.literal('creationTime')).or(z.literal('lastReply'))
